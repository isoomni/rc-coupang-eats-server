...
+ 유저 정보 조회 api 개발 및 업로드

+ 유저 정보 수정 api 개발 및 업로드

+ 클라이언트에게 이미지가 잘 뜨지 않는 부분 수정 요청 받아서 수정 후 확인 재요청

+ 클라이언트에게 유저 관련 api 상세 사항 전달
...
+ 클라이언트 요청으로 카테고리 조회(검색창 있는 페이지(검색 기능 X)) api 우선 개발 완료

+ 카트 조회, 등록, 수정, 삭제 api 에 jwt 적용
...
+ 홈 화면, 식당 조회 시에 유저 배달 주소 반경 내에 있는 식당으로 쿼리 작성하기 (jwt 적용)

+ validation 처리 post에서 꼼꼼하게 해주기.(get에서는 idx 값이 유효한지 정도로)

+ transaction 처리 해주기.

+ 25개 api 만들고, 소셜 로그인까지 구현하는 것을 우선 목표로
  + api 25개 완성되면 한나에게 보고

---
### 2021-08-10 진행상황

+ 특정 유저 즐겨찾기 페이지 조회 API(jwt) 개발 완료

+ 개발 도중 이슈
  + 재학중인 학교 졸업 관련 문제로 멘탈이 완전히 나가서 api 생산을 거의 하지 못했습니다. 내일 최대한 생산성을 끌어올릴 계획입니다..
  
### 2021-08-11 진행상황

+ api 25개 개발 완료

  + 즐겨찾기 등록, 삭제 api 개발 완료

  + 리뷰 등록, 수정, 삭제 api 개발 완료

  + 유저 탈퇴 api 개발 완료

  + 배달 주소 조회, 등록, 수정 삭제 api 개발 완료

+ 홈 화면, 카테고리 별 식당 조회, 식당 리뷰 조회 등 유저 api 생성 전 개발한 api에도 jwt 적용

+ 식당과 사용자 주소 위경도 이용해서 거리 기준으로 식당 보여주도록 쿼리 수정
  
  + 홈 화면 api, 식당 조회 api, 즐겨찾기 api 에 적용 (즐겨찾기의 경우 거리가 멀면 -> '해당 위치에서 주문할 수 없는 식당입니다.' 출력)
  
### 2021-08-12 진행상황

+ 개발 완료한 API 서버에서 POSTMAN 확인 완료

+ post api validation 처리 완료

+ 명세서 업데이트

+ 카트 등록 API 수정 - 등록이 일부만 할 수 있게 되어 있는데 GET에 있는 칼럼 전부 등록 할 수 있도록 수정해야 함.
